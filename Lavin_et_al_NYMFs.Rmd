...
Fig2a =  ggplot() + 
  ...
  geom_histogram(data=parent_size, aes(x=Parent_spawning_TL_cm), binwidth = 5, col = "black", fill = "grey28", alpha = .3) +
  geom_line(data=Fig.2.values, aes(x = range, y = mid, col = variable), size = .9) +
  geom_ribbon(data=Fig.2.values, aes(x = range, ymin = lower, ymax = upper, fill = variable), alpha = 0.4) +
  scale_x_continuous(breaks = seq(0,80,5), limits = c(25,80)) +
  scale_y_continuous(expand = c(0,0)) +
  scale_color_manual(values = c("#CC79A7", "#56B4E9"), labels = c("Assigned parents", "Sampled adults")) +
  scale_fill_manual(values = c("#CC79A7", "#56B4E9"), labels = c("Assigned parents", "Sampled adults")) +
...
R = ggplot(R.success.ratio, aes(x = range, y = `50%`)) +
  geom_ribbon(aes(ymin = `2.5%`, ymax = `97.5%`), fill = "#56B4E9", alpha = .4) +
  geom_line(col = "#56B4E9") +
  geom_segment(aes(x = 38, y = 0, xend = 38, yend = R.success.ratio %>% filter(range == 38.0 ) %>% pull(`50%`)), lty = "dotted", col = "#56B4E9") +
  geom_segment(aes(x = 60, y = 0, xend = 60, yend = R.success.ratio %>% filter(range == 60 ) %>% pull(`50%`)), lty = "dotted", col = "#56B4E9") +
...
R.contributions = ggplot(c.ratio, aes(x = range, y = `50%`)) +
  ...
  geom_segment(aes(x = 60, y = 0, xend = 60, yend = c.ratio %>% filter(range == 60.0) %>% pull(`50%`)), lty = "dotted") +
  scale_x_continuous(expand= c(0,0), breaks = c(30, 38, 50, 60, 70, 80)) +
  scale_y_continuous(expand= c(0,0)) +
...
ntmr.plot = ggplot(ntmr.zones[[1]] %>% spread(key = quantile, value = value), 
    ...
    geom_histogram(data = data.frame(x = ntmr$TOTAL.LENGTH), aes(x = x, y=..density..), 
                   binwidth=5, colour="black", fill="grey28", alpha = .3) + 
    geom_ribbon(aes(ymin = `2.5%`, ymax = `97.5%`), fill = "#009E73", alpha = .5) +
