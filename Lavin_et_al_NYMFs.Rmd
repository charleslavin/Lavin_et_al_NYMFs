...
bind_rows(
  ...
  R.success.ratio %>% filter(range >= 60 ) %>% mutate(c025 = cumsum(`2.5%`), c500 = cumsum(`50%`), c975 = cumsum(`97.5%`)) %>%
...
```{r}
bind_rows(c.ratio %>% filter(range <= 38 ) %>% mutate(c025 = cumsum(`2.5%`), c500 = cumsum(`50%`), c975 = cumsum(`97.5%`)) %>%
  filter(range == 38) %>% dplyr::select(range, c025, c500, c975),
c.ratio %>% filter(range >= 60 ) %>% mutate(c025 = cumsum(`2.5%`), c500 = cumsum(`50%`), c975 = cumsum(`97.5%`)) %>%
  filter(range == 80) %>% dplyr::select(range, c025, c500, c975)) %>%
  gather(key = key, value = value, -range) %>% spread(key = range, value = value) %>% 
  rename(NYMFs = `38`, BOFFFs = `80`) %>% 
...
```
## Cumulatively, in an unfished population of coral trout NYMFFs contribute 4 (3.5 - 4.2 95% CI) times more to local recruitment than BOFFFs.
...
```{r}
c.ratio %>%  mutate(c025 = cumsum(`2.5%`) / sum(`2.5%`), c500 = cumsum(`50%`) / sum(`50%`), c975 = cumsum(`97.5%`) / sum(`97.5%`)) %>% filter(range == 60 )
```
